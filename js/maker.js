...
ubuntu.line = function(val, release) {
  return "" + val + " http://mirrors.skyshe.cn/ubuntu " + release + " main universe restricted multiverse\n";
};
...
raspbian.line = function(val, flag, release) {
  return "" + val + " http://mirrors.skyshe.cn/raspbian/raspbian/" + " " + release + " main contrib non-free rpi\n";
};
...
debian.line = function(val, flag, release) {
  return "" + val + " http://mirrors.skyshe.cn/debian" + flag + " " + release + " main non-free contrib\n";
};
...
deepin.line = function(val, release) {
  return "" + val + " http://mirrors.skyshe.cn/deepin " + release + " main non-free\n";
};
...
epel.singleBlock = function(name, url, version) {
  return "[" + name + "]\nname=Extra Packages for Enterprise Linux " + version + " - $basearch " + (name.toUpperCase()) + "\nbaseurl=http://mirrors.skyshe.cn/epel/" + version + "/$basearch" + url + "\nenabled=1\ngpgcheck=1\ngpgkey=http://mirrors.skyshe.cn/epel/RPM-GPG-KEY-EPEL-" + version + "\n\n";
};
...
fedora.singleBlock = function(name, url, system) {
  return "[" + name + "]\nname=Fedora $releasever - $basearch\nbaseurl=http://mirrors.skyshe.cn/fedora/" + system + "/$releasever/" + url + "/\nenabled=1\ngpgcheck=1\ngpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-fedora-$basearch\n\n";
};
...
build.archlinux = function(what) {
  return "Server = http://mirrors.skyshe.cn/archlinux/$repo/os/$arch";
};
